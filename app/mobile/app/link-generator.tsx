...
export default function LinkGeneratorScreen() {
  ...
  const handleShare = async () => {
    const url =
      canonicalData ||
      `https:// StellarFoundry.to/${form.destination}/${form.amount}?asset=${recipientAssetCode}${form.memo ? `&memo=${encodeURIComponent(form.memo)}` : ""}`;
    try {
      await Share.share({
        message: `Pay me via  StellarFoundry:\n${url}`,
      });
    } catch (error: any) {
