...
// ── Theme imports ────────────────────────────────────────────────────────────
import { StellarFoundryThemeProvider, useTheme } from "../src/theme/ThemeContext";
import {
  LightTheme,
  DarkTheme,
...
describe("Theme System v2", () => {
  describe("Token Engine", () => {
    ...
    it("ThemeRegistry contains all four themes", () => {
      ...
      expect(ThemeRegistry[" StellarFoundry-blue"]).toBe(StellarFoundryBlueTheme);
  ...
  describe("ThemeProvider snapshots", () => {
    ...
    it("renders with wrapped provider", async () => {
      ...
      await act(async () => {
        tree = create(
          <StellarFoundryThemeProvider profileId="test-user">
            <ThemePreview />
          </StellarFoundryThemeProvider>,
        );
      });
