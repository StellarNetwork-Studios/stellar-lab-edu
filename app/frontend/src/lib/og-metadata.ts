...
export const SITE_NAME = " StellarFoundry";
...
export function getSiteUrl(): string {
  return (
    ...
    process.env.NEXT_PUBLIC_StellarFoundry_API_URL?.replace(/\/$/, "").replace(
      /:4000$/,
      ":3000",
    ) ||
    "http://localhost:3000"
...
export async function fetchPaymentMeta(params: {
  ...
  try {
    const apiBase =
      process.env.StellarFoundry_INTERNAL_API_URL?.replace(/\/$/, "") ||
...
export function buildPaymentTitle(meta: SafePaymentMeta): string {
  switch (meta.state) {
    ...
    default:
      return `Payment link —  StellarFoundry`;
  }
}
...
export const FALLBACK_PAYMENT_METADATA = {
  ...
  description:
    "This payment link is unavailable, expired, or invalid. Visit  StellarFoundry to create a new one.",
