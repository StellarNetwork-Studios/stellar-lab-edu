...
import { getStellarFoundryApiBase } from "@/lib/api";
...
const PUBLIC_KEY_STORAGE_CANDIDATES = [
  " StellarFoundry.publicKey",
  " StellarFoundry.walletPublicKey",
  "walletPublicKey",
  "publicKey",
];
...
function resolveAnalyticsPublicKey(): string {
  ...
  const fromEnv = process.env.NEXT_PUBLIC_StellarFoundry_ANALYTICS_PUBLIC_KEY?.trim();
...
export async function fetchAnalytics(range: DateRange): Promise<AnalyticsData> {
  ...
  const url = new URL(`${getStellarFoundryApiBase()}/analytics/report`);
...
export async function exportAnalyticsReport(
  ...
  const publicKey = resolveAnalyticsPublicKey();
  const { startDate, endDate, interval } = rangeToWindow(range);
  const url = new URL(`${getStellarFoundryApiBase()}/analytics/export`);
  url.searchParams.set("publicKey", publicKey);
  url.searchParams.set("startDate", startDate);
  url.searchParams.set("endDate", endDate);
  url.searchParams.set("interval", interval);
  ...
  const fallbackName = ` StellarFoundry-analytics-report.${format}`;
