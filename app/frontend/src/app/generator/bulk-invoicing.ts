...
export const TEMPLATE_STORAGE_KEY = " StellarFoundry.bulkInvoice.templates.v2";
export const CUSTOMER_STORAGE_KEY = " StellarFoundry.bulkInvoice.customers.v2";
...
export const DEFAULT_TEMPLATES: InvoiceTemplate[] = [
  {
    ...
    lineItems: [
      {
        id: "line-hosting-retainer",
        description: "Hosting retainer",
        quantity: 1,
        unitPrice: 120,
      },
      {
        id: "line-support-hours",
...
function toBulkCsvDraftRow(
  columns: BulkCsvColumns,
  index: number,
): BulkCsvDraftRow {
...
export function validateBulkCsvDraftRow(row: BulkCsvDraftRow): BulkCsvDraftRow {
  ...
  if (!ASSET_CODE_PATTERN.test(nextRow.asset)) {
    nextRow.errors.push(
      "Asset codes must be 1-12 uppercase letters or numbers.",
    );
  }

  if (!nextRow.username && !nextRow.destination) {
    nextRow.errors.push(
      "Add either a  StellarFoundry username or a Stellar destination.",
    );
  }

  if (
    nextRow.destination &&
    !STELLAR_ADDRESS_PATTERN.test(nextRow.destination)
  ) {
  ...
  if (nextRow.acceptedAssets) {
    ...
    if (invalidAcceptedAssets) {
      nextRow.errors.push(
        "Accepted assets must use pipe-separated asset codes like XLM|USDC.",
      );
    }
  }
...
export function parseBulkInvoiceCsv(csvContent: string): BulkCsvParseResult {
  ...
  if (lines.length === 0) {
    return {
      fileErrors: [
        "Upload a CSV file with a header row and at least one invoice row.",
      ],
      rows: [],
    };
  }
  ...
  const unsupportedHeaders = rawHeaders.filter(
    (header, index) => headers[index] === null,
  );
  const fileErrors =
