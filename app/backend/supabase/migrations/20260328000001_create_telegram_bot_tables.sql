...
-- ---------------------------------------------------------------------------
-- Maps Telegram user IDs to  StellarFoundry public keys for notification delivery.
-- Users link their Telegram account by starting a conversation with the bot
-- and providing their  StellarFoundry public key or username.
...
CREATE TABLE IF NOT EXISTS telegram_user_mappings (
  ...
  public_key TEXT NOT NULL,                   --  StellarFoundry public key (G...)
...
COMMENT ON TABLE telegram_user_mappings IS
  'Maps Telegram users to  StellarFoundry accounts for real-time payment notifications.';

COMMENT ON COLUMN telegram_user_mappings.is_verified IS 
  'Whether the user has confirmed the Telegram- StellarFoundry account link';
