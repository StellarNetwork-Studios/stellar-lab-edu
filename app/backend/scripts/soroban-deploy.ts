...
function parseArgs(argv: string[]): Args {
  ...
  const wasm =
    map.get("wasm") ??
    "app/contract/target/wasm32-unknown-unknown/release/ StellarFoundry.wasm";
  const contractName = map.get("contract-name") ?? " StellarFoundry";
  if (!source) {
...
async function main() {
  ...
  const contractId =
    run("soroban", deployCommand, args.dryRun) || "<contract-id>";
  ...
  const artifactPath = path.join(
    outputDir,
    `${args.network}-${args.contractName}.json`,
  );
  writeFileSync(artifactPath, JSON.stringify(artifact, null, 2));

  if (args.registryUrl) {
    const response = await fetch(
      `${args.registryUrl.replace(/\/$/, "")}/contracts/registry/publish`,
      {
        method: "POST",
        headers: {
  ...
  process.stdout.write(
    `${JSON.stringify({ artifactPath, artifact }, null, 2)}\n`,
  );
}
