...
pg_dump -U postgres  StellarFoundry > backup_before_crash_reporting.sql

# Run migration
psql -U postgres -d  StellarFoundry -f src/crash-reporting/migrations/001_create_crash_reporting_tables.sql

# Verify tables
psql -U postgres -d  StellarFoundry -c "\dt crash_*"
```
...
```bash
# Drop tables
psql -U postgres -d  StellarFoundry -c "DROP TABLE IF EXISTS crash_reports CASCADE;"
psql -U postgres -d  StellarFoundry -c "DROP TABLE IF EXISTS crash_reporting_settings CASCADE;"

# Restore from backup if needed
psql -U postgres -d  StellarFoundry < backup_before_crash_reporting.sql
```
