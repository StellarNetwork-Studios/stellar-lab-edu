...
export class AnalyticsService {
  ...
  async exportReport(
    ...
    const report: AnalyticsReport = rpcReport ?? {
      summary: this.buildSummary(payments),
      assetDistribution: this.buildAssetDistribution(payments),
      timeSeries: this.buildTimeSeries(payments, interval),
      window: {
  ...
  buildCsvReport(
    ...
    const lines: string[] = [];
    lines.push(` StellarFoundry_analytics_report_type,${reportType}`);
    ...
    lines.push(
      `successful_transactions,${report.summary.successfulTransactions}`,
    );
    ...
    lines.push(
      `average_transaction_usd,${report.summary.averageTransactionUsd}`,
    );
    lines.push("");
  ...
  buildPdfReport(
    ...
    const lines: string[] = [
      ` StellarFoundry ${reportType.toUpperCase()} Report`,
  ...
  private async fetchAggregatedReportViaRpc(
    ...
    const [summaryResult, assetsResult, timeSeriesResult] = await Promise.all([
      client.rpc(" StellarFoundry_analytics_summary", {
        p_public_key: publicKey,
        p_start_date: startIso,
        p_end_date: endIso,
        p_organization_id: organizationId ?? null,
      }),
      client.rpc(" StellarFoundry_analytics_asset_distribution", {
        p_public_key: publicKey,
        p_start_date: startIso,
        p_end_date: endIso,
        p_organization_id: organizationId ?? null,
      }),
      client.rpc(" StellarFoundry_analytics_time_series", {
        p_public_key: publicKey,
        p_start_date: startIso,
        p_end_date: endIso,
        p_interval: interval,
  ...
  private buildAssetDistribution(
    ...
    return Array.from(map.entries())
      .map(([asset, value]) => ({
        ...
        percentage:
          totalVolume > 0 ? this.round2((value.volume / totalVolume) * 100) : 0,
  ...
  private buildTimeSeries(
    ...
    payments.forEach((item) => {
      ...
      current.assets[item.asset] =
        (current.assets[item.asset] ?? 0) + item.amountUsd;
