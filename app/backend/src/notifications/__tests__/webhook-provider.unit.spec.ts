...
describe("WebhookProvider", () => {
  ...
  describe("send", () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          headers: expect.objectContaining({
            ...
            "X- StellarFoundry-Event": "payment.received",
          }),
        }),
      );
    ...
    it("should include HMAC signature in header", async () => {
      ...
      const signature = headers["X- StellarFoundry-Signature"];
    ...
    it("should include delivery ID and timestamp headers", async () => {
      ...
      expect(headers["X- StellarFoundry-Delivery"]).toMatch(/^wh_\d+_[a-z0-9]+$/);
      expect(headers["X- StellarFoundry-Timestamp"]).toMatch(
  ...
  describe("verifySignature", () => {
    it("should verify valid signature", async () => {
      ...
      const headers = call[1].headers;
      const signature = headers["X- StellarFoundry-Signature"];
      const timestamp = headers["X- StellarFoundry-Timestamp"];
