...
export class TelegramBotService implements OnModuleInit, OnModuleDestroy {
  ...
  private registerCommands(): void {
    ...
    this.bot.start(async (ctx) => {
      ...
      if (existingMapping) {
        await ctx.reply(
          `✅ Your Telegram account is already linked to  StellarFoundry!\n\n` +
      ...
      await ctx.reply(
        `👋 Welcome to  StellarFoundry Notifications Bot!\n\n` +
    ...
    this.bot.command("status", async (ctx) => {
      ...
      if (!mapping) {
        await ctx.reply(
          "❌ Your Telegram account is not linked to any  StellarFoundry account.\n\n" +
      ...
      await ctx.reply(
        `📊 Your  StellarFoundry Link Status\n\n` +
    ...
    this.bot.command("unlink", async (ctx) => {
      ...
      if (!mapping) {
        await ctx.reply("❌ No  StellarFoundry account is linked.");
        return;
      }
      ...
      await ctx.reply(
        "✅ Your  StellarFoundry account has been disconnected.\n\n" +
    ...
    this.bot.command("settings", async (ctx) => {
      ...
      if (!mapping) {
        await ctx.reply("❌ No  StellarFoundry account linked. Use /start first.");
        return;
      }
