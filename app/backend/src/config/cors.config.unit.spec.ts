...
describe("buildCorsOptions", () => {
  ...
  describe("production — static origins", () => {
    const opts = buildCorsOptions({
      nodeEnv: "production",
      allowedOrigins: [
        "https:// StellarFoundry.to",
        "https://app. StellarFoundry.to",
      ],
    });

    it("allows a listed origin", async () => {
      await expect(
        resolveOrigin(opts, "https:// StellarFoundry.to"),
      ).resolves.toBe(true);
    });

    it("allows a second listed origin", async () => {
      await expect(
        resolveOrigin(opts, "https://app. StellarFoundry.to"),
      ).resolves.toBe(true);
    });
  ...
  describe("production — Vercel preview URLs", () => {
    const opts = buildCorsOptions({
      nodeEnv: "production",
      allowedOrigins: ["https:// StellarFoundry.to"],
      vercelProject: " StellarFoundry-frontend",
    });
    ...
    it("allows a valid Vercel preview URL", async () => {
      await expect(
        resolveOrigin(
          opts,
          "https:// StellarFoundry-frontend-abc123-team.vercel.app",
        ),
      ).resolves.toBe(true);
    });
    ...
    it("blocks a URL that tries to spoof the project name", async () => {
      await expect(
        resolveOrigin(
          opts,
          "https://evil- StellarFoundry-frontend-abc.vercel.app",
        ),
      ).rejects.toThrow("Origin not allowed");
    });
