...
export class StellarController {
  ...
  @ApiOperation({
    ...
    description:
      "Uses the same pipeline as POST /transactions/compose against  StellarFoundry_CONTRACT_ID.",
  })
  async sorobanPreflight(@Body() body: SorobanPreflightDto) {
    const contractId = this.appConfig.StellarFoundryContractId;
    if (!contractId?.trim()) {
      throw new ServiceUnavailableException({
        code: "CONTRACT_NOT_CONFIGURED",
        message:
          "Set  StellarFoundry_CONTRACT_ID to enable Soroban preflight simulation.",
      });
    }
