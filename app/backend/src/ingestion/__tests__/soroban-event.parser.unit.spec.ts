...
import {
  StellarFoundry_EVENT_SCHEMA_CONTRACTS,
  StellarFoundry_EVENT_SCHEMA_VERSION,
  StellarFoundry_EVENT_TOPICS,
} from "../event-schema";
...
describe("SorobanEventParser", () => {
  ...
  describe("EscrowDeposited", () => {
    it("parses canonical testnet topics with schema versioned payload", () => {
      const topics = [
        symVal(StellarFoundry_EVENT_TOPICS.escrow),
      ...
      const data = mapVal({
        ...
        schema_version: nativeToScVal(StellarFoundry_EVENT_SCHEMA_VERSION, {
          type: "u32",
        }),
      ...
      expect(result.topicNamespace).toBe(StellarFoundry_EVENT_TOPICS.escrow);
      expect(result.schemaVersion).toBe(StellarFoundry_EVENT_SCHEMA_VERSION);
  ...
  describe("error cases", () => {
    ...
    it("returns null for unsupported schema versions", () => {
      const topics = [
        symVal(StellarFoundry_EVENT_TOPICS.privacy),
  ...
  describe("event schema contracts", () => {
    it("locks canonical topics and deterministic payload key order", () => {
      expect(StellarFoundry_EVENT_SCHEMA_CONTRACTS.EscrowDeposited).toEqual({
  ...
  describe("replay metadata extraction", () => {
    it("extracts contractLedgerSequence when ledger_sequence is in payload", () => {
      ...
      const data = mapVal({
        ...
        schema_version: nativeToScVal(StellarFoundry_EVENT_SCHEMA_VERSION, { type: "u32" }),
        timestamp: nativeToScVal(1700000000n, { type: "u64" }),
        token: addressVal(TOKEN),
      });
