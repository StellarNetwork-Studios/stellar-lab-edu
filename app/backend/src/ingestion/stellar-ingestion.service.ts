...
/**
 * Listens to Horizon SSE streams for a  StellarFoundry Soroban contract.
 *
 * Responsibilities:
...
export class StellarIngestionService implements OnModuleInit, OnModuleDestroy {
  ...
  private async handleRecord(
    ...
    if (!event) {
      // Unrecognised or non- StellarFoundry event; still advance cursor.
  ...
  private async persistEvent(event: StellarFoundryContractEvent): Promise<void> {
