...
interface TopicLayout {
  eventName: SorobanEventType;
  topicNamespace: StellarFoundryEventTopic | "LEGACY";
 ...
 *  Topic[0] = stable  StellarFoundry testnet namespace (for example TOPIC_ESCROW)
...
export class SorobanEventParser {
   ...
   */
  parse(raw: RawHorizonContractEvent): StellarFoundryContractEvent | null {
    try {
  ...
  private resolveTopicLayout(topics: xdr.ScVal[]): TopicLayout | null {
    ...
    const canonicalTopics = new Set<string>(
      Object.values(StellarFoundry_EVENT_TOPICS),
    );
    if (canonicalTopics.has(first)) {
      const second = topics[1] ? this.decodeSymbol(topics[1]) : null;
      if (!second || !(second in StellarFoundry_EVENT_SCHEMA_CONTRACTS))
        return null;

      const contract =
        StellarFoundry_EVENT_SCHEMA_CONTRACTS[
          second as keyof typeof StellarFoundry_EVENT_SCHEMA_CONTRACTS
        ];
      if (contract.topic !== first) return null;

      return {
        eventName: second as SorobanEventType,
        topicNamespace: first as StellarFoundryEventTopic,
        indexedOffset: 2,
      };
    }

    if (first in StellarFoundry_EVENT_SCHEMA_CONTRACTS) {
      return {
  ...
  private isCompatibleSchemaVersion(
    ...
    const contract =
      StellarFoundry_EVENT_SCHEMA_CONTRACTS[
        eventName as keyof typeof StellarFoundry_EVENT_SCHEMA_CONTRACTS
      ];
