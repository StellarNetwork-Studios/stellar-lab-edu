...
export class ContractViewsService {
  ...
  private async fetchFeeConfig(): Promise<FeeConfigView> {
    ...
    // When  StellarFoundry_CONTRACT_ID is not set (e.g. local dev) we return safe
    // defaults so the frontend can still render.
    const contractId =
      this.configService.get<string>("StellarFoundry_CONTRACT_ID") ??
      process.env["StellarFoundry_CONTRACT_ID"];

    if (!contractId) {
      this.logger.warn(
        " StellarFoundry_CONTRACT_ID not set — returning default fee config",
      );
