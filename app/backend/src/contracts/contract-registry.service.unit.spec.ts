...
describe("ContractRegistryService", () => {
  ...
  it("publishes and returns the active registry", async () => {
    ...
    expect(result.data.StellarFoundry).toEqual(
  ...
  it("rolls back to a previous contract version", async () => {
    ...
    const result = await service.rollback({ name: " StellarFoundry", version: 1 });
    expect(result.data.StellarFoundry).toEqual(
      expect.objectContaining({ id: "C123", wasmHash: "abc123", version: 1 }),
    );
  });
  ...
  it("throws when rolling back a missing version", async () => {
    await expect(
      service.rollback({ name: " StellarFoundry", version: 99 }),
  ...
  describe("Dual-read finalization", () => {
    it("finalizes dual-read by clearing previousContractId", async () => {
      ...
      const result = await service.finalizeDualRead(" StellarFoundry");
      ...
      expect(result.data.StellarFoundry).toBeDefined();
    });
    ...
    it("throws when not in dual-read window", async () => {
      ...
      mockClient.rpc.mockResolvedValue({
        data: null,
        error: {
          message: "Registry entry for stellar-foundry is not in a dual-read transition window",
        },
      });

      await expect(service.finalizeDualRead(" StellarFoundry")).rejects.toThrow(
        "Registry entry for stellar-foundry is not in a dual-read transition window",
      );
    });
  });
