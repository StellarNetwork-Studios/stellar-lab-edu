...
This directory contains end-to-end smoke tests that validate the deployment health and critical functionality of the StellarFoundry backend API. These tests are designed to run automatically after deployment to ensure the system is functioning correctly.
...
SMOKE_TEST_BASE_URL=https://api. StellarFoundry.io npm run test:smoke:all

# Test against staging
SMOKE_TEST_BASE_URL=https://api-staging. StellarFoundry.io npm run test:smoke:all
```
...
- **Production**: `https://api. StellarFoundry.io`
- **Staging**: `https://api-staging. StellarFoundry.io`
...
import {
  SmokeTestReporter,
  runSmokeTest,
} from "./common/utils/smoke-test-reporter.util";

const reporter = new SmokeTestReporter(
  "production",
  "https://api. StellarFoundry.io",
);
reporter.start();
