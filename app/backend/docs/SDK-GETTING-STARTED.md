# StellarFoundry SDK — Getting Started Guide

> A complete guide for integrating StellarFoundry into your JavaScript or TypeScript application.
...
- A ** StellarFoundry API key** (optional — gives higher rate limits)
...
```bash
npm install @ StellarFoundry/sdk
# or
pnpm add @ StellarFoundry/sdk
# or
yarn add @ StellarFoundry/sdk
```

### Option B: Use the lightweight HTTP client (zero dependencies)

The StellarFoundry API is a standard REST API. You can use `fetch` directly:
...
```typescript
//  StellarFoundry.config.ts
export const StellarFoundry_CONFIG = {
  // Base URL — change to https://api. StellarFoundry.example.com for production
  baseUrl: process.env.StellarFoundry_BASE_URL || "http://localhost:3000",

  // Optional API key for higher rate limits
  apiKey: process.env.StellarFoundry_API_KEY || "",
...
| ----------------------- | -------- | ----------------------- | ------------------------------ |
| ` StellarFoundry_BASE_URL` | No       | `http://localhost:3000` | API base URL                   |
| ` StellarFoundry_API_KEY`  | No       | —                       | API key for higher rate limits |
...
StellarFoundry uses API keys for authentication. API keys are optional for public endpoints but give you:
...
class StellarFoundryClient {
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      ...
      throw new StellarFoundryError(error);
    }
...
// Custom error class
class StellarFoundryError extends Error {
  code: string;
  requestId?: string;
...
```typescript
const client = new StellarFoundryClient(
  "http://localhost:3000",
...
```
https://app. StellarFoundry.example.com/pay?amount=50.5000000&asset=XLM&username=alice_123
```
...
async function requestWithRetry<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        error instanceof StellarFoundryError &&
...
try {
  ...
  if (error instanceof StellarFoundryError) {
    switch (error.code) {
      case "USERNAME_TAKEN":
...
StellarFoundry uses cursor-based pagination for list endpoints. Pass the `cursor` from a previous response to get the next page.

```typescript
async function getAllTransactions(
  client: StellarFoundryClient,
  accountId: string,
): Promise<Transaction[]> {
...
When you register a webhook, StellarFoundry signs every payload with a secret. Verify signatures to ensure authenticity.
...
function verifyWebhookSignature(
  ...
  signature: string, // x- StellarFoundry-signature header
...
// Express example
app.post("/webhooks/ StellarFoundry", (req, res) => {
  const signature = req.headers["x- StellarFoundry-signature"] as string;
  const raw = JSON.stringify(req.body); // use raw body parser in production
  const secret = process.env.StellarFoundry_WEBHOOK_SECRET!;
