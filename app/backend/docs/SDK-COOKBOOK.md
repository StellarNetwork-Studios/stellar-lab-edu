...
const response = await fetch(
  `http://localhost:3000/payment-links/status?${params}`,
);
...
const response = await fetch(`http://localhost:3000/webhooks/${publicKey}`, {
  ...
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    webhookUrl: "https://example.com/webhooks/ StellarFoundry",
...
function verify StellarFoundryWebhook(
  rawBody: string,
...
function handlePaymentReceived(data: any) {
  console.log(
    `Payment received: ${data.amount} ${data.asset} from ${data.from}`,
  );
}
...
function handleRecurringPayment(data: any) {
  console.log(
    `Recurring payment executed: period ${data.periodNumber}, tx: ${data.transactionHash}`,
  );
}

function handleRecurringFailure(data: any) {
  console.error(
    `Recurring payment failed: ${data.failureReason}, retries: ${data.retryCount}`,
  );
}
...
```typescript
// app/api/webhooks/ StellarFoundry/route.ts
import { NextRequest, NextResponse } from "next/server";
import { createHmac } from "crypto";

export async function POST(request: NextRequest) {
  const rawBody = await request.text();
  const signature = request.headers.get("x- StellarFoundry-signature") || "";
  const secret = process.env.StellarFoundry_WEBHOOK_SECRET!;
...
for (const tx of transactions) {
  console.log(
    `${tx.timestamp} | ${tx.type} | ${tx.amount} ${tx.assetCode || "XLM"}`,
  );
...
const response = await fetch(
  `http://localhost:3000/username?publicKey=${publicKey}`,
);
...
for (const c of creators) {
  console.log(
    `@${c.username}: $${c.transactionVolume} (${c.transactionCount} txs)`,
  );
}
```
...
```typescript
const response = await fetch(
  `http://localhost:3000/links/recurring/${linkId}/cancel`,
  {
    method: "POST",
  },
);
...
```typescript
const response = await fetch(
  `http://localhost:3000/links/recurring/${linkId}`,
  {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
const response = await fetch("http://localhost:3000/stellar/path-preview", {
  ...
  body: JSON.stringify({
    ...
    sourceAssets: [{ code: "XLM" }, { code: "AQUA", issuer: "..." }],
  }),
});
...
for (const path of paths) {
  console.log(
    `Send ${path.sourceAmount} ${path.sourceAsset} → Receive 10 USDC`,
  );
...
```typescript
const response = await fetch(
  "http://localhost:3000/stellar/path-preview/strict-send",
  {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
for (const path of quote.paths) {
  console.log(
    `Send ${path.sourceAmount} XLM (max: ${path.sourceAmountWithSlippage})`,
  );
}
...
```typescript
const response = await fetch(
  `http://localhost:3000/marketplace/${listingId}/bid`,
  {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
```typescript
const response = await fetch(
  `http://localhost:3000/api-keys/${apiKeyId}/rotate`,
  {
    method: "POST",
  },
);
