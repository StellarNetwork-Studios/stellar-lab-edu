# StellarFoundry API — Error Contract & Troubleshooting Guide

Every non-2xx response from the StellarFoundry backend follows a single JSON shape.
...
| Code                      | HTTP Status | Description                                      |
| ------------------------- | ----------- | ------------------------------------------------ |
| `CONTRACT_NOT_CONFIGURED` | 503         | StellarFoundry_CONTRACT_ID is not set on the server |
...
| `CONTRACT_NOT_CONFIGURED`    | 503  | Soroban        | Server configuration issue; contact the StellarFoundry team             |
...
```typescript
  ...
  const error = err.response?.data?.error;
  console.error(
    ` StellarFoundry error [${error.code}]: ${error.message}\n` +
...
5. If you need higher limits, contact the StellarFoundry team for a custom rate limit.
...
2. StellarFoundry caches responses for 60 seconds — don't bypass caching by varying query params.
...
1. The ` StellarFoundry_CONTRACT_ID` environment variable is not set on the server.
2. This is a server-side configuration issue — contact the StellarFoundry team.
3. In development, set it in your `.env` file:
   ```env
    StellarFoundry_CONTRACT_ID=CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAD2KM
   ```
