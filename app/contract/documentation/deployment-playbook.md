# StellarFoundry Soroban Deployment Playbook

This playbook defines the repeatable deployment process for StellarFoundry Soroban contracts.
...
```bash
export  StellarFoundry_TESTNET_RPC_URL="https://soroban-testnet.stellar.org"
export  StellarFoundry_TESTNET_PASSPHRASE="Test SDF Network ; September 2015"
export  StellarFoundry_TESTNET_IDENTITY=" StellarFoundry-testnet"

export  StellarFoundry_MAINNET_RPC_URL="https://mainnet.stellar.org"
export  StellarFoundry_MAINNET_PASSPHRASE="Public Global Stellar Network ; September 2015"
export  StellarFoundry_MAINNET_IDENTITY=" StellarFoundry-mainnet"
```
...
soroban config network add testnet \
  --rpc-url "$ StellarFoundry_TESTNET_RPC_URL" \
  --network-passphrase "$ StellarFoundry_TESTNET_PASSPHRASE"

soroban config network add mainnet \
  --rpc-url "$ StellarFoundry_MAINNET_RPC_URL" \
  --network-passphrase "$ StellarFoundry_MAINNET_PASSPHRASE"
```

Suggested identity bootstrap flow:

```bash
soroban config identity generate "$ StellarFoundry_TESTNET_IDENTITY"
soroban config identity generate "$ StellarFoundry_MAINNET_IDENTITY"
```
...
stellar contract deploy \
  --wasm target/wasm32v1-none/release/ StellarFoundry.wasm \
  --source "$ StellarFoundry_TESTNET_IDENTITY" \
  --network testnet
...
For each environment, record the deployed StellarFoundry contract address and release details in [environment-registry.toml](environment-registry.toml).
