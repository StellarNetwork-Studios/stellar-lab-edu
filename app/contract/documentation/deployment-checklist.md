# StellarFoundry Deployment Checklist

This checklist is the release gate for any StellarFoundry deployment change. Use it for testnet validation first, then repeat the exact validated steps for mainnet.
...
If the PR touches an event payload, also run the relevant snapshot test(s) in [contracts/ StellarFoundry/src/test.rs](../contracts/ StellarFoundry/src/test.rs) and confirm the payload keys remain locked.
