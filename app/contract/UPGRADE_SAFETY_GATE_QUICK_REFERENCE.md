...
```rust
set_upgrade_window(env, caller, start_epoch, end_epoch) -> Result<(),  StellarFoundryError>
```
...
```rust
start_upgrade(env, caller, new_version) -> Result<(),  StellarFoundryError>
```
...
```rust
upgrade(env, caller, new_wasm_hash) -> Result<(),  StellarFoundryError>
```
...
```rust
complete_upgrade(env, caller, new_version) -> Result<u32,  StellarFoundryError>
```
