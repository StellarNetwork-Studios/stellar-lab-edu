...
This document describes the normalized authorization guard system for the StellarFoundry contract. All mutating entry points must use the shared guard helpers defined in `admin.rs` to ensure consistent security checks.
...
2. **Add the guard call** at the beginning of your function:
   ```rust
   pub fn new_operation(env: Env, caller: Address, ...) -> Result<(), StellarFoundryError> {
...
4. **Implement the test function**:
   ```rust
   fn test_new_operation(env: &Env, caller: &Address) -> Result<(), crate::errors::StellarFoundryError> {
       StellarFoundryContract::new_operation(env.clone(), caller.clone(), ...)
   }
   ```
