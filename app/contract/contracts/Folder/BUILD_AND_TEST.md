...
# Build the  StellarFoundry contract
cargo build --package  StellarFoundry

# Build with release optimizations
cargo build --release --package  StellarFoundry
```
...
```bash
# Run all tests
cargo test --package  StellarFoundry
...
cargo test --package  StellarFoundry -- --include-ignored
```
...
```
Compiling  StellarFoundry v0.1.0 (...)
Finished `dev` profile [unoptimized + debuginfo] target(s) in XX.XXs

Running unittests src/lib.rs (target/debug/deps/ StellarFoundry-xxxxxxxxxxxxxxx)
...
1. `env.register_at(contract_id,  StellarFoundryContract, ())` — swaps the WASM in-place on the same address
