...
pub const ESCROW_ID_DOMAIN_TAG: &[u8] = b" StellarFoundry::ESCROW_ID::v1";
...
/// # Errors
///
/// - [` StellarFoundryError::InvalidAmount`] if `amount < 0`.
/// - [` StellarFoundryError::InvalidSalt`] if `salt.len() > 1024`.
pub fn derive_escrow_id(
    ...
    if salt.len() > MAX_SALT_LEN {
        return Err( StellarFoundryError::InvalidSalt);
    }
...
pub const PARTIAL_ESCROW_ID_DOMAIN_TAG: &[u8] = b" StellarFoundry::PARTIAL_ESCROW_ID::v1";
...
/// # Errors
///
/// - [` StellarFoundryError::InvalidAmount`] if `amount_due < 0` or `initial_payment < 0`.
/// - [` StellarFoundryError::InvalidSalt`] if `salt.len() > 1024`.
