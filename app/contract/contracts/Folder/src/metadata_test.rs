...
use crate::{
    ...
    types::{
     ...
     PauseFlag, StellarFoundryContract, StellarFoundryContractClient,
};
...
fn setup() -> (Env,  StellarFoundryContractClient<'static>) {
    let env = Env::default();
    env.mock_all_auths();
...
fn metadata_contract_id_differs_across_deployments() {
    ...
    let id_a = env.register( StellarFoundryContract, ());
    let id_b = env.register( StellarFoundryContract, ());

    let client_a =  StellarFoundryContractClient::new(&env, &id_a);
    let client_b =  StellarFoundryContractClient::new(&env, &id_b);
