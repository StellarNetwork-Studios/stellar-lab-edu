...
use errors::StellarFoundryError;
use storage::*;
use types::{
...
///  StellarFoundry Privacy Contract
///
/// Soroban smart contract providing escrow, privacy controls, and X-Ray-style amount
/// commitments for the  StellarFoundry platform. See the contract README for main flows.
///
/// ## Asset Support
///
...
/// [`StellarFoundryContract::get_escrow_operation_limits`]. The current supported
/// envelopes are:
...
///   [`OperationPaused`](errors::StellarFoundryError::OperationPaused) /
///   [`ContractPaused`](errors::StellarFoundryError::ContractPaused).
...
#[contract]
pub struct StellarFoundryContract;

#[contractimpl]
#[allow(clippy::too_many_arguments)]
impl StellarFoundryContract {
    ...
    pub fn withdraw(
        ...
        to: Address,
        salt: Bytes,
    ) -> Result<bool, StellarFoundryError> {
    ...
    /// For boolean on/off privacy, prefer [`set_privacy`]( StellarFoundryContract::set_privacy).
    ///
    ...
    pub fn set_privacy(env: Env, owner: Address, enabled: bool) -> Result<(), StellarFoundryError> {
    ...
    /// off-chain or via [`create_amount_commitment`]( StellarFoundryContract::create_amount_commitment).
    ///
    /// # Arguments
    /// * `env` - The contract environment
    ...
    pub fn activate_emergency_mode(env: Env, caller: Address) -> Result<(), StellarFoundryError> {
    ...
    pub fn cleanup_escrow(env: Env, commitment: BytesN<32>) -> Result<(), StellarFoundryError> {
        admin::guard_initialized(&env)?;
    ...
    pub fn extend_escrow_ttl(env: Env, commitment: BytesN<32>) -> Result<(), StellarFoundryError> {
        admin::guard_initialized(&env)?;
    ...
    pub fn dispute(env: Env, commitment: BytesN<32>) -> Result<(), StellarFoundryError> {
    ...
    pub fn estimate_deposit_resources(
        ...
        salt_bytes: u32,
        arbiter_count: u32,
    ) -> Result<EscrowOperationEstimate, StellarFoundryError> {
    ...
    /// # Errors
    /// * `Unauthorized` - Caller is not the admin, or admin not set
    pub fn pause_features(env: Env, caller: Address, mask: u64) -> Result<(), StellarFoundryError> {
        admin::guard_admin_config(&env)?;
    ...
    /// # Errors
    /// * `Unauthorized` - Caller is not the admin, or admin not set
    pub fn unpause_features(env: Env, caller: Address, mask: u64) -> Result<(), StellarFoundryError> {
        admin::guard_admin_config(&env)?;
    ...
    pub fn register_hook(env: Env, hook_contract: Address) -> Result<(), StellarFoundryError> {
        admin::guard_initialized(&env)?;
        hook::register_hook(&env, hook_contract)
    }

    /// Unregister a hook contract.
    pub fn unregister_hook(env: Env, hook_contract: Address) -> Result<(), StellarFoundryError> {
        admin::guard_initialized(&env)?;
    ...
    ///
    /// # Security
    /// Updates the contract's executable code. Call [`migrate`]( StellarFoundryContract::migrate)
