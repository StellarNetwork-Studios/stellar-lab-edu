...
pub struct BatchItemResult {
    pub index: u32,
    pub success: bool,
    /// Non-zero on failure; maps to ` StellarFoundryError` discriminant.
    pub error_code: u32,
}
...
pub fn batch_create(
...
) -> Result<Vec<BatchItemResult>,  StellarFoundryError> {
    caller.require_auth();
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        if item.amount <= 0 {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarFoundryError::InvalidAmount as u32 });
            continue;
        }

        if get_escrow(env, &item.escrow_id).is_some() {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarFoundryError::EscrowExists as u32 });
            continue;
        }
...
pub fn batch_release(
    ...
    for (i, id) in escrow_ids.iter().enumerate() {
        ...
        let mut entry = match get_escrow(env, &id) {
            Some(e) => e,
            None => {
                results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarFoundryError::EscrowNotFound as u32 });
                continue;
            }
        };

        if entry.status != EscrowStatus::Pending {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarFoundryError::AlreadySpent as u32 });
            continue;
        }

        if entry.expires_at > 0 && now >= entry.expires_at {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarFoundryError::EscrowExpired as u32 });
            continue;
        }
...
pub fn batch_refund(
    ...
    for (i, id) in escrow_ids.iter().enumerate() {
        ...
        if entry.expires_at == 0 || now < entry.expires_at {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarFoundryError::EscrowNotExpired as u32 });
            continue;
        }
