...
//!    consumed once. Replaying the same nonce fails with [` StellarFoundryError::NonceAlreadyUsed`].
//!
//! 2. **Expiry window** — the signed message carries a `valid_until` ledger
//!    timestamp. Submitting after that timestamp fails with
//!    [` StellarFoundryError::SignatureExpired`].
//!
...
/// | Error | Condition |
/// |-------|-----------|
/// | [` StellarFoundryError::NonceAlreadyUsed`] | `(signer, nonce)` already consumed |
/// | [` StellarFoundryError::SignatureExpired`] | `env.ledger().timestamp() >= valid_until` |
