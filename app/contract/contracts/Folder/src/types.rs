//! Types used in the  StellarFoundry storage layer and contract logic.
//!
...
impl FeeRatio {
    ...
    pub fn validate(&self) -> Result<(), StellarFoundryError> {
        if self.numerator == 0 {
            return Ok(());
        }
        if self.denominator == 0 || self.numerator > self.denominator {
            return Err(StellarFoundryError::InvalidFeeConfiguration);
        }
        Ok(())
    }
}
...
///
/// Returned by [` StellarFoundryContract::get_escrow_details`] instead of the raw
...
/// Deployment metadata returned by [`crate:: StellarFoundryContract::get_deployment_metadata`].
///
