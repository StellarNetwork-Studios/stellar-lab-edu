...
//! 2. Performs an in-place upgrade to ` StellarFoundryContract` (v1) via `env.register_at`,
//!    then calls `migrate()`.
...
use crate::{
    ...
    types::FeeConfig,
    EscrowStatus,  StellarFoundryContract,  StellarFoundryContractClient,
};
use soroban_sdk::{
...
///
/// Key differences from the current ` StellarFoundryContract`:
...
/// Swap the legacy WASM for the current ` StellarFoundryContract` on the same address,
/// returning a ready-to-use client pointing at the upgraded contract.
fn upgrade_to_current<'a>(env: &'a Env, contract_id: &Address) ->  StellarFoundryContractClient<'a> {
    env.register_at(contract_id,  StellarFoundryContract, ());
     StellarFoundryContractClient::new(env, contract_id)
}
...
fn upgrade_harness_non_admin_migrate_fails() {
    ...
    assert_eq!(
        result,
        Err(Ok( StellarFoundryError::InsufficientRole)),
...
fn seed_admin_role<'a>(
    ...
    contract_id: &Address,
    admin: &Address,
) ->  StellarFoundryContractClient<'a> {
...
fn upgrade_safety_gate_invariant_failure_deterministic() {
    ...
    assert_eq!(
        result,
        Err(Ok( StellarFoundryError::InternalError)),
