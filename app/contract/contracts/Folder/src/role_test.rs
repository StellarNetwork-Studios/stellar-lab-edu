use crate::{errors::StellarFoundryError, storage, test_context::TestContext, types::Role};
...
fn test_admin_transfer_requires_acceptance_and_can_be_cancelled() {
    ...
    assert!(matches!(
        cancelled_accept,
        Err(Ok(StellarFoundryError::NoPendingAdminTransfer))
    ));
...
fn test_cannot_revoke_admin_role_from_current_admin() {
    ...
    assert!(matches!(
        result,
        Err(Ok(StellarFoundryError::InvalidRoleState))
    ));
}
