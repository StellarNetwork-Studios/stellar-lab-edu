use crate::{errors:: StellarFoundryError, events, storage, types::HookEventKind};
...
pub fn register_hook(env: &Env, hook_contract: Address) -> Result<(),  StellarFoundryError> {
    ...
    if hooks.contains(hook_contract.clone()) {
        return Err( StellarFoundryError::HookAlreadyRegistered);
    }
...
pub fn unregister_hook(env: &Env, hook_contract: Address) -> Result<(),  StellarFoundryError> {
    ...
    if !found {
        return Err( StellarFoundryError::HookNotRegistered);
    }
...
pub fn assert_not_reentrant(env: &Env) -> Result<(),  StellarFoundryError> {
    if storage::get_reentrancy_guard(env) {
        return Err( StellarFoundryError::ReentrancyDetected);
    }
    Ok(())
}
