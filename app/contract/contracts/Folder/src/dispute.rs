...
pub fn resolve_expired_dispute(
    ...
    let expiry = get_dispute_expiry(env, &commitment_bytes).ok_or(StellarFoundryError::NoDisputeExpiry)?;
    let now = env.ledger().timestamp();
    if now < expiry.expires_at {
        return Err(StellarFoundryError::DisputeNotExpired);
    }
...
fn resolve_expiry_recipient(
    entry: &EscrowEntry,
    action: DisputeExpiryAction,
) -> Result<Address, StellarFoundryError> {
    match action {
