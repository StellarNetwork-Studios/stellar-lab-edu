...
pub fn register_ephemeral_key(
    ...
    if expected_stealth != stealth_address {
        return Err( StellarFoundryError::StealthAddressMismatch);
    }

    // Reject duplicate stealth addresses (replay protection).
    if get_stealth_escrow(env, &stealth_address).is_some() {
        return Err( StellarFoundryError::StealthAddressAlreadyUsed);
    }
...
pub fn stealth_withdraw(
    ...
    let entry =
        get_stealth_escrow(env, &stealth_address).ok_or(StellarFoundryError::StealthEscrowNotFound)?;
...
/// clean a still-`Pending` entry returns [`AlreadySpent`](StellarFoundryError::AlreadySpent)
...
///
/// # Errors
/// - [`StealthEscrowNotFound`](StellarFoundryError::StealthEscrowNotFound) – no entry for the address.
/// - [`AlreadySpent`](StellarFoundryError::AlreadySpent) – entry is not in a terminal state.
...
pub fn cleanup_stealth_escrow(
    ...
    let entry = get_stealth_escrow(env, &stealth_address)
        .ok_or(StellarFoundryError::StealthEscrowNotFound)?;
    ...
    match entry.status {
        ...
        _ => Err(StellarFoundryError::AlreadySpent),
    }
}
