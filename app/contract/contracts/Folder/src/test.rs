use crate::{
    ...
    EscrowEntry, EscrowStatus,  StellarFoundryContract,  StellarFoundryContractClient,
};
...
///  StellarFoundry contract integration tests.
///
...
#[contract]
pub struct LegacyStellarFoundryContract;

#[contractimpl]
impl LegacyStellarFoundryContract {
    pub fn initialize(env: Env, admin: Address) -> Result<(),  StellarFoundryError> {
        if crate::storage::get_admin(&env).is_some() {
            return Err( StellarFoundryError::AlreadyInitialized);
        }
    ...
    ) -> Result<BytesN<32>,  StellarFoundryError> {
        if crate::admin::is_paused(&env) {
            return Err( StellarFoundryError::ContractPaused);
        }
        if crate::storage::is_feature_paused(&env, PauseFlag::Deposit) {
            return Err( StellarFoundryError::OperationPaused);
        }
...
fn setup<'a>() -> (Env,  StellarFoundryContractClient<'a>) {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register( StellarFoundryContract, ());
    let client =  StellarFoundryContractClient::new(&env, &contract_id);
    (env, client)
}
...
fn test_set_privacy_already_set_fails() {
    ...
    assert_contract_error(result,  StellarFoundryError::PrivacyAlreadySet);
}
...
fn test_double_withdrawal_fails() {
    ...
    assert_contract_error(second_result,  StellarFoundryError::AlreadySpent);
}
...
fn test_invalid_salt_fails() {
    ...
    assert_contract_error(result,  StellarFoundryError::CommitmentNotFound);
}
...
fn test_zero_amount_fails() {
    ...
    assert_contract_error(result,  StellarFoundryError::InvalidAmount);
}
...
fn test_canonical_error_code_ranges() {
    ...
    assert_eq!( StellarFoundryError::InvalidAmount as u32, 100);
    assert_eq!( StellarFoundryError::InvalidSalt as u32, 101);
    assert_eq!( StellarFoundryError::InvalidPrivacyLevel as u32, 102);

    // Auth/admin failures (200-299)
    assert_eq!( StellarFoundryError::Unauthorized as u32, 200);
    assert_eq!( StellarFoundryError::AlreadyInitialized as u32, 201);
    assert_eq!( StellarFoundryError::InsufficientRole as u32, 202);

    // State/escrow/commitment violations (300-399)
    assert_eq!( StellarFoundryError::ContractPaused as u32, 300);
    assert_eq!( StellarFoundryError::PrivacyAlreadySet as u32, 301);
    assert_eq!( StellarFoundryError::CommitmentNotFound as u32, 302);
    assert_eq!( StellarFoundryError::CommitmentAlreadyExists as u32, 303);
    assert_eq!( StellarFoundryError::AlreadySpent as u32, 304);
    assert_eq!( StellarFoundryError::InvalidCommitment as u32, 305);
    assert_eq!( StellarFoundryError::CommitmentMismatch as u32, 306);
    assert_eq!( StellarFoundryError::EscrowExpired as u32, 307);
    assert_eq!( StellarFoundryError::EscrowNotExpired as u32, 308);
    assert_eq!( StellarFoundryError::InvalidOwner as u32, 309);
    assert_eq!( StellarFoundryError::PayloadTooLarge as u32, 329);
    assert_eq!( StellarFoundryError::TooManyFeeRecipients as u32, 330);
    assert_eq!( StellarFoundryError::TooManyTokens as u32, 331);

    // Internal/unexpected conditions (900-999)
    assert_eq!( StellarFoundryError::InternalError as u32, 900);
}
...
fn test_deposit_rejects_large_but_otherwise_valid_salt_payloads() {
    ...
    assert_contract_error(result, StellarFoundryError::PayloadTooLarge);
}
...
fn test_initialize_twice_fails() {
    ...
    assert_contract_error(result,  StellarFoundryError::AlreadyInitialized);
}
...
fn test_config_mutation_before_initialize_fails_deterministically() {
    ...
    assert_contract_error(result,  StellarFoundryError::Unauthorized);
}
...
fn test_set_privacy_same_value_fails() {
    ...
    assert_contract_error(second,  StellarFoundryError::PrivacyAlreadySet);
}
...
//     assert_contract_error(result,  StellarFoundryError::ContractPaused);
// }
...
fn test_deposit_with_commitment_fails_when_paused() {
    ...
    assert_contract_error(result,  StellarFoundryError::OperationPaused);
}
...
fn test_set_paused_by_non_admin_fails() {
    ...
    assert_contract_error(result,  StellarFoundryError::InsufficientRole);
}
...
fn test_upgrade_by_admin() {
    ...
    match result {
        ...
        Err(Ok(contract_error)) => {
            ...
            assert_ne!(
                contract_error,
                 StellarFoundryError::Unauthorized,
...
fn test_upgrade_migration_preserves_legacy_escrow_data() {
    ...
    let contract_id = env.register(LegacyStellarFoundryContract, ());
    let legacy_client = LegacyStellarFoundryContractClient::new(&env, &contract_id);
    ...
    env.register_at(&contract_id,  StellarFoundryContract, ());
    let client =  StellarFoundryContractClient::new(&env, &contract_id);
...
fn test_withdrawal_fails_after_expiry() {
    ...
    assert_eq!(res, Err(Ok(crate::errors:: StellarFoundryError::EscrowExpired)));
}
...
fn test_refund_successful() {
    ...
    assert_eq!(res, Err(Ok(crate::errors:: StellarFoundryError::EscrowNotExpired)));
...
fn test_refund_unauthorized_fails() {
    ...
    assert_eq!(res, Err(Ok(crate::errors:: StellarFoundryError::InvalidOwner)));
}
...
fn test_double_refund_fails() {
    ...
    let res = client.try_refund(&commitment, &owner);
    assert_eq!(res, Err(Ok(crate::errors:: StellarFoundryError::AlreadySpent)));
}
...
fn test_dispute_fails_without_arbiter() {
    ...
    assert_eq!(res, Err(Ok(crate::errors:: StellarFoundryError::NoArbiter)));
}
...
fn test_dispute_fails_on_non_pending_status() {
    ...
    assert_eq!(
        res,
        Err(Ok(crate::errors:: StellarFoundryError::InvalidDisputeState))
    );
}
...
fn test_resolve_dispute_fails_for_non_arbiter() {
    ...
    assert_eq!(res, Err(Ok(crate::errors:: StellarFoundryError::NotArbiter)));
}
...
fn test_set_dispute_expiry_action_requires_admin() {
    ...
    assert_eq!(
        res,
        Err(Ok(crate::errors:: StellarFoundryError::InsufficientRole))
    );
...
fn test_resolve_expired_dispute_fails_before_timeout() {
    ...
    assert_eq!(
        res,
        Err(Ok(crate::errors:: StellarFoundryError::DisputeNotExpired))
    );
}
...
fn test_cross_asset_zero_amount_edge_case() {
    ...
    assert_eq!(result, Err(Ok( StellarFoundryError::InvalidAmount)));
}
...
fn test_partial_payment_overpayment_rejected() {
    ...
    assert_contract_error(result,  StellarFoundryError::Overpayment);
}
...
fn test_partial_payment_terminal_state_rejected() {
    ...
    let result = client.try_partial_payment(&commitment, &payer, &100);
    assert_contract_error(result,  StellarFoundryError::AlreadySpent);
}
...
fn test_deposit_with_arbiters_rejects_zero_threshold() {
    ...
    assert_eq!(result.unwrap_err().unwrap(), crate::errors:: StellarFoundryError::InvalidThreshold);
}
...
fn test_deposit_with_arbiters_rejects_duplicate_arbiters() {
    ...
    assert_eq!(result.unwrap_err().unwrap(), crate::errors:: StellarFoundryError::DuplicateArbiter);
}
...
fn test_deposit_with_arbiters_rejects_above_supported_upper_bound() {
    ...
    assert_contract_error(result, StellarFoundryError::TooManyArbiters);
}
...
fn test_deposit_partial_duplicate_commitment_rejected() {
    ...
    assert_eq!(
        ...
        crate::errors:: StellarFoundryError::CommitmentAlreadyExists
    );
}
