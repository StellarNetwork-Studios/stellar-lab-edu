...
fn compute_expires_at(env: &Env, timeout_secs: u64) -> Result<u64,  StellarFoundryError> {
    if timeout_secs == 0 {
    ...
    if expires_at == u64::MAX {
        return Err( StellarFoundryError::InvalidTimeout);
    }
...
fn estimate_deposit_resources(
    ...
    if salt_bytes > MAX_OPERATION_SALT_BYTES {
        return Err(StellarFoundryError::PayloadTooLarge);
    }
    if arbiter_count > MAX_ARBITERS {
        return Err(StellarFoundryError::TooManyArbiters);
    }
...
fn estimate_withdraw_resources(
    ...
    if fee_recipient_count > MAX_WITHDRAW_FEE_RECIPIENTS {
        return Err(StellarFoundryError::TooManyFeeRecipients);
    }
...
fn validate_deposit_resources(salt: &Bytes, arbiter_count: u32) -> Result<(), StellarFoundryError> {
    let estimate = estimate_deposit_resources(salt.len(), arbiter_count)?;
    if estimate.token_count > MAX_SUPPORTED_TOKEN_COUNT {
        return Err(StellarFoundryError::TooManyTokens);
    }
...
pub fn deposit(
    ...
    if has_escrow(env, &commitment_bytes) {
        return Err( StellarFoundryError::CommitmentAlreadyExists);
    }
...
pub fn deposit_with_arbiters(
    ...
    if arbiters.is_empty() || threshold == 0 {
        return Err( StellarFoundryError::InvalidThreshold);
    }
    ...
    for i in 0..arbiter_count {
        for j in (i + 1)..arbiter_count {
            if arbiters.get_unchecked(i) == arbiters.get_unchecked(j) {
                return Err( StellarFoundryError::DuplicateArbiter);
            }
        }
    }
...
pub fn partial_payment(
    ...
    let mut entry: EscrowEntry =
        get_escrow(env, &commitment_bytes).ok_or( StellarFoundryError::CommitmentNotFound)?;

    // INV-5: terminal states are final
    if entry.status != EscrowStatus::Pending {
        return Err( StellarFoundryError::AlreadySpent);
    }
    ...
    if payment_amount > remaining {
        return Err( StellarFoundryError::Overpayment);
    }
...
pub fn withdraw(env: &Env, amount: i128, to: Address, salt: Bytes) -> Result<bool,  StellarFoundryError> {
    if amount <= 0 {
        return Err( StellarFoundryError::InvalidAmount);
    }
    ...
    let (commitment, commitment_bytes, entry): (BytesN<32>, Bytes, EscrowEntry) =
        if let Some(entry) = get_escrow(env, &commitment_bytes) {
            ...
            let entry = get_escrow(env, &legacy_commitment_bytes)
                .ok_or( StellarFoundryError::CommitmentNotFound)?;
    ...
    if entry.status != EscrowStatus::Pending {
        ...
        if entry.status == EscrowStatus::Disputed {
            return Err( StellarFoundryError::InvalidDisputeState);
        }
        return Err( StellarFoundryError::AlreadySpent);
    }
    ...
    if !is_within_window(env, &entry) {
        return Err( StellarFoundryError::EscrowExpired);
    }

    if entry.amount_due != amount {
        return Err( StellarFoundryError::InvalidCommitment);
    }
...
pub fn refund(env: &Env, commitment: BytesN<32>, caller: Address) -> Result<(),  StellarFoundryError> {
    caller.require_auth();
    ...
    if !is_expired(env, &entry) {
        return Err( StellarFoundryError::EscrowNotExpired);
    }

    if caller != entry.owner {
        return Err( StellarFoundryError::InvalidOwner);
    }
...
pub fn extend_escrow_ttl(env: &Env, commitment: BytesN<32>) -> Result<(),  StellarFoundryError> {
    ...
    if !has_escrow(env, &commitment_bytes) {
        return Err( StellarFoundryError::CommitmentNotFound);
    }
...
pub fn cleanup_escrow(env: &Env, commitment: BytesN<32>) -> Result<(),  StellarFoundryError> {
    ...
    match entry.status {
        ...
        _ => Err( StellarFoundryError::AlreadySpent), // Reuse error or add a more specific one if needed
    }
}
...
pub fn dispute(env: &Env, commitment: BytesN<32>) -> Result<(),  StellarFoundryError> {
    ...
    let arbiter = entry.arbiter.as_ref().ok_or( StellarFoundryError::NoArbiter)?;
...
pub fn resolve_dispute(
    ...
    if !is_authorized {
        return Err( StellarFoundryError::NotArbiter);
    }
...
pub fn vote_for_dispute(
    ...
    if entry.arbiter_threshold == 0 {
        return Err( StellarFoundryError::NoArbiter);
    }
    ...
    if !is_arbiter {
        return Err( StellarFoundryError::NotAnArbiter);
    }

    // Guard: arbiter must not have already voted
    if has_dispute_vote(env, &commitment_bytes, &caller) {
        return Err( StellarFoundryError::ArbiterAlreadyVoted);
    }
...
pub fn resolve_dispute_multi_sig(
    ...
    if vote_count < entry.arbiter_threshold {
        return Err( StellarFoundryError::InsufficientVotes);
    }
