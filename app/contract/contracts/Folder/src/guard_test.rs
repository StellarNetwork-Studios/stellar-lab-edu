use crate::{
    ...
    storage::{PauseFlag},
    StellarFoundryContract, StellarFoundryContractClient,
};
...
struct GuardTestCase {
    name: &'static str,
    test_fn: fn(&Env, &Address) -> Result<(), StellarFoundryError>,
    should_succeed: bool,
}
...
fn setup_initialized_contract(env: &Env) -> (Address, StellarFoundryContractClient) {
...
fn test_initialized_contract_allows_guard_initialized(env: &Env, _caller: &Address) -> Result<(), StellarFoundryError> {
    let admin = Address::generate(env);
    let contract_id = env.register(StellarFoundryContract, ());
    let client = StellarFoundryContractClient::new(env, &contract_id);
    client.initialize(&admin);
...
fn test_emergency_mode_blocks_deposits(env: &Env, _caller: &Address) -> Result<(), StellarFoundryError> {
    let admin = Address::generate(env);
    let contract_id = env.register(StellarFoundryContract, ());
    let client = StellarFoundryContractClient::new(env, &contract_id);
    client.initialize(&admin);
    ...
    match result {
        Err(_) => Ok(()),
        _ => Err(StellarFoundryError::ContractPaused),
    }
}

fn test_emergency_mode_does_not_block_withdrawals(env: &Env, _caller: &Address) -> Result<(), StellarFoundryError> {
    let admin = Address::generate(env);
    let contract_id = env.register(StellarFoundryContract, ());
    let client = StellarFoundryContractClient::new(env, &contract_id);
    client.initialize(&admin);
    ...
    match result {
        Ok(_) => Ok(()),
        _ => Err(StellarFoundryError::InternalError),
    }
}

fn test_emergency_mode_blocks_admin_config(env: &Env, _caller: &Address) -> Result<(), StellarFoundryError> {
    let admin = Address::generate(env);
    let contract_id = env.register(StellarFoundryContract, ());
    let client = StellarFoundryContractClient::new(env, &contract_id);
    client.initialize(&admin);
...
fn test_global_pause_blocks_operations(env: &Env, _caller: &Address) -> Result<(), StellarFoundryError> {
    let admin = Address::generate(env);
    let contract_id = env.register(StellarFoundryContract, ());
    let client = StellarFoundryContractClient::new(env, &contract_id);
    client.initialize(&admin);
