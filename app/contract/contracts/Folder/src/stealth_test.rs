...
use crate::{
    errors:: StellarFoundryError, stealth, types::StealthDepositParams, EscrowStatus,  StellarFoundryContract,
     StellarFoundryContractClient,
};
use soroban_sdk::{
...
fn test_register_wrong_stealth_address_fails() {
    ...
    assert_eq!(err,  StellarFoundryError::StealthAddressMismatch);
}
...
fn test_register_duplicate_stealth_address_fails() {
    ...
    assert_eq!(err, StellarFoundryError::StealthAddressAlreadyUsed);
}
...
fn test_stealth_double_withdraw_fails() {
    ...
    assert_eq!(err, StellarFoundryError::StealthEscrowNotFound);
}
...
fn test_stealth_withdraw_after_expiry_fails() {
    ...
    assert_eq!(err,  StellarFoundryError::EscrowExpired);
}
...
fn test_stealth_register_zero_amount_fails() {
    ...
    assert_eq!(err,  StellarFoundryError::InvalidAmount);
}
...
fn test_stealth_register_fails_when_paused() {
    ...
    assert_eq!(err,  StellarFoundryError::ContractPaused);
}
