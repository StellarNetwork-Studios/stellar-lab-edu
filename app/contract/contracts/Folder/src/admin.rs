use crate::errors::StellarFoundryError;
use crate::events::{
...
pub fn require_initialized(env: &Env) -> Result<(), StellarFoundryError> {
    if storage::is_initialized(env) {
        Ok(())
    } else {
        Err(StellarFoundryError::Unauthorized)
    }
}
...
fn current_admin(env: &Env) -> Result<Address, StellarFoundryError> {
    let admin = storage::get_admin(env).ok_or(StellarFoundryError::InvalidRoleState)?;
    let roles = storage::get_roles(env, &admin);
    if roles.contains(Role::Admin) {
        Ok(admin)
    } else {
        Err(StellarFoundryError::InvalidRoleState)
    }
}
...
pub fn require_any_role(
    ...
    Err(StellarFoundryError::InsufficientRole)
}

/// Require that the caller is an Admin.
pub fn require_admin(env: &Env, caller: &Address) -> Result<(), StellarFoundryError> {
...
pub fn grant_role(
    ...
    if target == admin && role == Role::Admin {
        return Err(StellarFoundryError::InvalidRoleState);
    }
...
pub fn set_admin(env: &Env, caller: Address, new_admin: Address) -> Result<(), StellarFoundryError> {
    require_admin(env, &caller)?;
...
pub fn accept_admin_transfer(env: &Env, caller: Address) -> Result<(), StellarFoundryError> {
    caller.require_auth();
    let new_admin =
        storage::get_pending_admin_transfer(env).ok_or(StellarFoundryError::NoPendingAdminTransfer)?;
    if caller != new_admin {
        return Err(StellarFoundryError::InsufficientRole);
    }
...
pub fn cancel_admin_transfer(env: &Env, caller: Address) -> Result<(), StellarFoundryError> {
    require_admin(env, &caller)?;
    if storage::get_pending_admin_transfer(env).is_none() {
        return Err(StellarFoundryError::NoPendingAdminTransfer);
    }
...
pub fn clear_roles(env: &Env, caller: Address, target: Address) -> Result<(), StellarFoundryError> {
    require_admin(env, &caller)?;
    let admin = current_admin(env)?;
...
pub fn set_paused(env: &Env, caller: Address, new_state: bool) -> Result<(), StellarFoundryError> {
...
pub fn migrate(env: &Env, caller: &Address) -> Result<u32, StellarFoundryError> {
    let from_version = get_version(env);
    if from_version == storage::LEGACY_CONTRACT_VERSION {
        caller.require_auth();

        let admin = storage::get_admin(env).ok_or(StellarFoundryError::Unauthorized)?;
        if admin != *caller {
            return Err(StellarFoundryError::InsufficientRole);
        }
    ...
    if from_version > storage::CURRENT_CONTRACT_VERSION {
        return Err(StellarFoundryError::InvalidContractVersion);
    }
    ...
    while version < storage::CURRENT_CONTRACT_VERSION {
        version = match version {
            ...
            _ => return Err(StellarFoundryError::InvalidContractVersion),
        };
    }
    ...
    if let Err(_msg) = storage::assert_post_upgrade_invariants(env) {
        env.panic_with_error(StellarFoundryError::InternalError);
    }
...
pub fn start_upgrade(
    ...
    if !storage::is_upgrade_window_active(env) {
        return Err(StellarFoundryError::UpgradeWindowNotActive);
    }

    if storage::is_upgrade_in_progress(env) {
        return Err(StellarFoundryError::UpgradeAlreadyInProgress);
    }
...
pub fn upgrade(
    ...
    if !storage::is_upgrade_in_progress(env) {
        return Err(StellarFoundryError::UpgradeNotInProgress);
    }
    ...
    let pending_hash =
        storage::get_pending_upgrade_wasm_hash(env).ok_or(StellarFoundryError::InternalError)?;

    if new_wasm_hash != pending_hash {
        return Err(StellarFoundryError::CommitmentMismatch);
    }
...
pub fn cancel_upgrade(env: &Env, caller: &Address) -> Result<(), StellarFoundryError> {
    require_admin(env, caller)?;
...
pub fn complete_upgrade(
    ...
    caller: &Address,
    new_version: u32,
) -> Result<u32, StellarFoundryError> {
    if !storage::is_upgrade_in_progress(env) {
        return Err(StellarFoundryError::UpgradeNotInProgress);
    }
    ...
    let pending_version =
        storage::get_pending_upgrade_version(env).ok_or(StellarFoundryError::InternalError)?;
    let pending_hash =
        storage::get_pending_upgrade_wasm_hash(env).ok_or(StellarFoundryError::InternalError)?;
    ...
    let actual_hash = storage::get_wasm_hash(env).ok_or(StellarFoundryError::InternalError)?;
    if actual_hash != pending_hash {
        return Err(StellarFoundryError::InternalError);
    }
...
#[allow(dead_code)]
pub fn require_not_paused(env: &Env) -> Result<(), StellarFoundryError> {
    if is_paused(env) {
...
pub fn require_not_emergency_mode(env: &Env) -> Result<(), StellarFoundryError> {
...
pub fn require_not_paused_global(env: &Env) -> Result<(), StellarFoundryError> {
    if is_paused(env) {
        return Err(StellarFoundryError::ContractPaused);
    }
    Ok(())
...
pub fn require_feature_not_paused(env: &Env, flag: crate::storage::PauseFlag) -> Result<(), StellarFoundryError> {
...
pub fn guard_deposit(env: &Env, pause_flag: crate::storage::PauseFlag) -> Result<(), StellarFoundryError> {
...
pub fn guard_withdraw(env: &Env, pause_flag: crate::storage::PauseFlag) -> Result<(), StellarFoundryError> {
    require_not_paused_global(env)?;
    require_feature_not_paused(env, pause_flag)?;
    crate::hook::assert_not_reentrant(env)?;
    Ok(())
...
///
/// Checks: global pause, feature pause, reentrancy.
pub fn guard_refund(env: &Env, pause_flag: crate::storage::PauseFlag) -> Result<(), StellarFoundryError> {
    require_not_paused_global(env)?;
    require_feature_not_paused(env, pause_flag)?;
    crate::hook::assert_not_reentrant(env)?;
    Ok(())
...
pub fn guard_dispute(env: &Env) -> Result<(), StellarFoundryError> {
    require_not_paused_global(env)?;
    crate::hook::assert_not_reentrant(env)?;
    Ok(())
}
...
pub fn guard_admin_config(env: &Env) -> Result<(), StellarFoundryError> {
    require_not_emergency_mode(env)?;
    crate::hook::assert_not_reentrant(env)?;
    Ok(())
}
...
pub fn guard_initialized(env: &Env) -> Result<(), StellarFoundryError> {
    require_initialized(env)?;
    crate::hook::assert_not_reentrant(env)?;
    Ok(())
}
...
///
/// Checks: global pause, feature pause, reentrancy.
pub fn guard_stealth(env: &Env, pause_flag: crate::storage::PauseFlag) -> Result<(), StellarFoundryError> {
    require_not_paused_global(env)?;
    require_feature_not_paused(env, pause_flag)?;
    crate::hook::assert_not_reentrant(env)?;
    Ok(())
...
pub fn set_per_asset_fee(
    ...
    if config.fee_bps > 10_000 || config.arbiter_bps > 10_000 {
        return Err(StellarFoundryError::InvalidAmount);
    }
    config.validate()?;
