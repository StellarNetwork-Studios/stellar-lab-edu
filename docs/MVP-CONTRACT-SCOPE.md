# MVP Contract Scope Decision: StellarFoundry Soroban Smart Contracts

## Overview

This document defines the Soroban smart contract scope for the StellarFoundry Minimum Viable Product (MVP). It outlines the core candidate modules, separates on-chain enforcement from off-chain validation, details the deployment design, and specifies versioning and operational safety requirements prior to mainnet launch.
...
- **Decision**: Deploy **one monolithic contract** (` StellarFoundryContract`).
- **Rationale**:
...
| Network                 | Horizon RPC Endpoint                    | Native Asset Address   | StellarFoundry Escrow Contract ID               |
| :---------------------- | :-------------------------------------- | :--------------------- | :------------------------------------------- |
...
> [!IMPORTANT]
> To comply with PR FE-31, both frontend and backend clients must consume these contract IDs from a centralized Contract Registry service, or fall back to the environment variable ` StellarFoundry_CONTRACT_ID` rather than embedding them directly in client source code.
