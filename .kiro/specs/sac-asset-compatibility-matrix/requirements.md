...
The SAC (Stellar Asset Contract) Asset Compatibility Matrix feature validates and documents contract behavior across XLM (the native Stellar asset) and common SAC tokens (USDC, AQUA, yXLM). It ensures that transfer semantics, decimal handling, amount normalization, and fee routing work consistently and correctly across all supported asset types. The feature also establishes clear rejection behavior for unsupported assets and invalid issuers, providing deterministic and testable guarantees for the StellarFoundry platform.
...
**User Story:** As a developer integrating with StellarFoundry, I want asset inputs to be normalized to a canonical form, so that XLM and SAC tokens are identified consistently regardless of how they are provided.
